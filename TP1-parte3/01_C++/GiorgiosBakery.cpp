...
mutex mtx_cout;  // Mutex para imprimir por consola
...
void master_baker() {
    while (true) {
        ...
        {
            lock_guard<mutex> lock_cout(mtx_cout);
...
void client(int id, int quantity) {
    ClientOrder* order = new ClientOrder{id, quantity};
    ...
    {
        unique_lock<mutex> lock(mtx_sales);
        cv_client_served.wait(lock, [&order] { return order->served; });
    }
